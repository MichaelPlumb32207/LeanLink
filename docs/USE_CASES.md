...
| UC-21·E12 | Innings = scoring arms only (D-038) | The line score shows **three** inning rows — FEC (T1), FL contributions (T2), OSINT (T3). **Party (T0)** is a muted pre-game context line (no lean). **Sunbiz** has no row: it renders as an "Identity enrichment" section **inside the FL contributions panel** ("N officers identified" + its runs + `match-sunbiz-entity` action). Its officer-count appears exactly once (in that section) — no D-036 duplication. |
