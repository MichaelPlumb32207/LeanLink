...
| UC-20·E8 | Re-fuse pending (background job, D-039) | A labeled-but-unfused population shows a "{N} voters behind {M} committees await re-fusion" strip (manager + box-score "on base"); **Re-fuse now** kicks off a background `committee_refuse` arm_run (no size cap — the old 150 inline cap + Grok-spend CLI hint are gone) and returns immediately ("started — watch the box score"). Progress shows live in the box score as "Committee re-fusion N/total"; the worker (`refuse-worker/[runId]`) processes pending committees single-pass with per-committee heartbeats, self-chaining past 800s via `arm_runs.meta`. On completion the pending count + ON BASE strip shrink on the next poll. |
...
| ID | Test | Expected |
|---|---|---|
| UC-21·H | Click each scoring inning row (FEC, FL contrib, OSINT) | Row toggles a detail panel (one open at a time; click again collapses): role + honest explainer, this arm's game log, and the arm's actions. **No funnel numbers** — the per-arm stats live once, in the line-score row (scoreboard single-source, D-036). Party is context (not clickable); Sunbiz is nested under FL (see ·E3/·E12). |
...
| UC-21·E3 | Sunbiz nested under FL (always, including first-run) | Opening FL contributions always shows the Identity enrichment section with `match-sunbiz-entity` — even on a fresh upload with zero Sunbiz events (`not_run`, zero officers). Stats/state may be empty; the **action surface is never activity-gated**. After a completed zero-settle run the explainer still carries the DEF-010 framing. No duplicated funnel. |
...
| UC-21·E7 | No step buttons (D-037) | Steps-1–5 / pipeline cards are **retired** — every former step action lives in an arm panel (or ON BASE for committees). Shared hook is still `use-evidence-actions` (same endpoints, 5,000/complete-rerun guards, confirm prompt). |
...
| UC-21·E12 | Innings = scoring arms only (D-038) | The line score shows **three** inning rows — FEC (T1), FL contributions (T2), OSINT (T3). **Party (T0)** is a muted pre-game context line (no lean). **Sunbiz** has no row: it always renders as an "Identity enrichment" section **inside the FL contributions panel** (action + zero-friendly "N officers identified" + runs), including on first-run/`not_run`. Officer-count appears exactly once (in that section) — no D-036 duplication. |
| UC-21·E15 | Refuse worker progress (D-039) | A live `committee_refuse` run shows "N voters re-fused · M pending at start" — **not** scoring-arm funnel terms (raw candidates / ID hits / lean signals). Progress bar is capped at 100% even if re-fusion overshoots kickoff `voters_pending`. `arm_runs.meta.processed_committees` is written on every heartbeat (resume-safe on hard kill). |
| UC-21·E13 | Click an inning → filters the voter list (D-038) | Clicking the FEC / FL contributions inning also sets the voter-list arm filter (`fec` / `fl_contrib`) via the existing `armFilters` (no new query); OSINT has no list filter; collapsing the row clears the filter. Expanding + filtering are one gesture. |
| UC-21·E14 | Tier-3 research-arms scaffold (ENH-022) | Beneath the OSINT (T3) row, indented **T3a–T3f** placeholder sub-rows show a **planned** badge and expand to a stub describing where the Grok research pass will be defined. Pure scaffold — no data, no runs, no spend; keyboard-operable like the innings. |
