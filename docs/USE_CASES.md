...
| T17.13 | Upload with unresolved FL committees | "Committee lean labels (N)" button badge + flag line: N committees · M eligible voters could gain a fused lean; label one committee → both counts shrink on the next poll (no re-pass needed for the counter). |
